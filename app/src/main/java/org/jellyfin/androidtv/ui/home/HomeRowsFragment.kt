		...
		lifecycleScope.launch(Dispatchers.IO) {			val currentUser = withTimeout(30.seconds) {
			...
			for (section in homesections) when (section) {
				HomeSectionType.LATEST_MEDIA -> {
					val views = userViewsRepository.views.firstOrNull()
					if (views != null) rows.add(helper.loadRecentlyAdded(views))
				}
			...
			withContext(Dispatchers.Main) {
				...
				view?.post {
					verticalGridView?.setItemSpacing((32 * resources.displayMetrics.density).toInt())
				}
